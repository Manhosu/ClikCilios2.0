...
export const PerformanceMonitor = ({ isVisible = false, position = 'top-right' }) => {
    ...
    return (<div className={`fixed ${positionClasses[position]} z-50 bg-black/80 backdrop-blur-sm rounded-lg border border-gray-700 text-white text-xs font-mono transition-all duration-300 ${isExpanded ? 'w-80' : 'w-16'}`}>
      <div className="p-2 cursor-pointer flex items-center justify-between" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex items-center space-x-2">
          <div className={`w-2 h-2 rounded-full bg-green-500`}/>
          {isExpanded && <span>Performance</span>}
        </div>
        {isExpanded && (<span className="text-gray-400">
            {isExpanded ? '−' : '+'}
          </span>)}
      </div>
      
      {isExpanded && (<div className="p-3 pt-0 space-y-2">
