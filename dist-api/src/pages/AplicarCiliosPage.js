...
import { getEstilosCilios, downloadProcessedImage } from '../services/aiService';
...
const AplicarCiliosPage = () => {
    ...
    const salvarImagemAutomaticamente = async (resultado) => {
        if (!autoSalvar || !user?.id || !resultado.imagemProcessada)
            return;
        try {
    ...
    const handleEstiloClick = useCallback((estiloId) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              <Button onClick={() => navigate('/dashboard')} variant="secondary" className="mr-4">
                ← Voltar
              </Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="card-elegant p-6">
              ...
              <div className="space-y-4">
                <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-primary-300 rounded-2xl p-8 text-center hover:border-primary-500 hover:bg-primary-50 transition-all cursor-pointer group">
                  {imagemOriginal ? (<div className="space-y-4">
                      <img src={imagemOriginal} alt="Imagem selecionada" className="max-h-48 mx-auto rounded-2xl shadow-lg group-hover:scale-105 transition-transform"/>
                      <p className="text-sm text-gray-600">✨ Clique para alterar a imagem</p>
                      {arquivoOriginal && (<div className="text-xs text-gray-500 bg-white/80 p-2 rounded-xl">
                ...
                <input ref={fileInputRef} type="file" accept="image/jpeg,image/jpg,image/png" onChange={handleFileUpload} className="hidden"/>
              </div>
            </div>
            ...
            <div className="card-elegant p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {estilosCilios.length > 0 ? (estilosCilios.map((estilo) => (<button key={estilo.id} onClick={() => {
                handleEstiloClick(estilo.id);
            }} className={`p-4 rounded-2xl border-2 transition-all hover:scale-105 group relative overflow-hidden ${estiloSelecionado === estilo.id
                ...
                : 'border-gray-200 hover:border-primary-300 hover:bg-primary-50'}`}>
                      ...
                      <div className="w-16 h-12 mx-auto mb-3 relative bg-gradient-to-r from-gray-100 to-gray-50 rounded-lg overflow-hidden shadow-inner">
                        <img src={estilo.overlayPath} alt={`Preview ${estilo.nome}`} className="w-full h-full object-contain filter drop-shadow-sm group-hover:scale-110 transition-transform" onLoad={() => { }} onError={(e) => {
                ...
                e.currentTarget.nextElementSibling?.classList.remove('hidden');
                      ...
                      {estiloSelecionado === estilo.id && (<div className="mt-3 text-xs text-primary-600 font-medium bg-primary-100 px-2 py-1 rounded-lg">
                          ✓ Selecionado
                        </div>)}
                    ...
                    </button>))) : (<div className="col-span-3 p-8 text-center text-gray-500">
            ...
            <div className="card-elegant p-6">
              <Button onClick={handleAplicarCilios} disabled={!arquivoOriginal || !estiloSelecionado || processando} variant={!arquivoOriginal || !estiloSelecionado || processando ? "secondary" : "primary"} className={`w-full py-4 text-lg shadow-elegant ${!arquivoOriginal || !estiloSelecionado || processando
            ...
            : 'hover:scale-105 transition-transform'}`}>
              ...
              {processando && (<div className="mt-6">
                  <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                    <div className="bg-gradient-to-r from-primary-500 to-secondary-500 h-3 rounded-full transition-all duration-300 shadow-sm" style={{ width: `${progresso}%` }}></div>
                  </div>
          ...
          <div className="space-y-6">
            <div className="card-elegant p-6">
              ...
              {!imagemOriginal && !resultado && (<div className="aspect-square bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl flex items-center justify-center">
              ...
              {imagemOriginal && !resultado && !processando && (<div className="aspect-square bg-gradient-to-br from-primary-50 to-secondary-50 rounded-2xl flex items-center justify-center border-2 border-dashed border-primary-300">
                  <div className="text-center text-gray-600">
                    <div className="text-6xl mb-4">⏳</div>
                    <p className="font-medium">Clique em "Aplicar Cílios" para ver o resultado</p>
                    {estiloSelecionado && (<p className="text-sm mt-3 text-primary-600 bg-white px-3 py-1 rounded-lg">
              ...
              {processando && (<div className="aspect-square bg-gradient-to-br from-primary-50 to-secondary-50 rounded-2xl flex items-center justify-center">
                  <div className="text-center text-gray-600">
                    <div className="text-6xl mb-4 animate-spin">🔄</div>
                    <p className="font-medium">Processando com IA...</p>
                    <div className="mt-6 w-64 bg-white rounded-full h-3 shadow-inner">
                      <div className="bg-gradient-to-r from-primary-500 to-secondary-500 h-3 rounded-full transition-all duration-300" style={{ width: `${progresso}%` }}></div>
                    </div>
              ...
              {resultado && resultado.status === 'concluido' && (<div className="space-y-6">
                  <div className="relative group">
                    {resultado.imagemProcessada ? (<img src={resultado.imagemProcessada} alt="Resultado com cílios aplicados" className="w-full rounded-2xl shadow-lg group-hover:shadow-xl transition-shadow" onLoad={() => { }} onError={() => { }}/>) : (<div className="w-full aspect-square bg-gradient-to-br from-yellow-50 to-orange-50 rounded-2xl flex items-center justify-center border border-yellow-200">
                        <div className="text-center text-yellow-700">
                          <div className="text-4xl mb-2">⚠️</div>
                          <p className="font-medium">Resultado sem imagem</p>
                          <p className="text-sm">Usando imagem original</p>
                          <img src={imagemOriginal || ''} alt="Imagem original" className="mt-4 max-w-full rounded-lg"/>
                        </div>
                      </div>)}
                    {resultado.metadata && (<div className="absolute top-3 right-3 bg-black/70 text-white text-xs px-3 py-1 rounded-xl backdrop-blur-sm">
                  ...
                  <div className="text-sm text-gray-600 bg-gradient-to-r from-gray-50 to-gray-100 p-4 rounded-2xl">
                    ...
                    {resultado.tempoProcessamento && (<p className="flex items-center">
                  ...
                  <div className="space-y-4">
                    <div className="flex space-x-4">
                      <Button onClick={handleDownload} variant="primary" className="flex-1 shadow-elegant hover:scale-105 transition-transform">
                        📥 Baixar Resultado
                      </Button>
                      <Button onClick={handleSalvarImagem} variant="secondary" disabled={salvandoImagem} className="flex-1 bg-gradient-to-r from-green-500 to-emerald-500 text-white hover:from-green-600 hover:to-emerald-600 disabled:opacity-50">
                        {salvandoImagem ? (<>
              ...
              {resultado && resultado.status === 'erro' && (<div className="aspect-square bg-gradient-to-br from-red-50 to-red-100 rounded-2xl flex items-center justify-center border border-red-200">
                  <div className="text-center text-red-600">
                    ...
                    <Button onClick={handleTryAgain} variant="primary" className="mt-6 shadow-elegant hover:scale-105 transition-transform">
                      🔄 Tentar Novamente
                    </Button>
                  </div>
                </div>)}
