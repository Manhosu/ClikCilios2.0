  ...
  try {
    ...
    if (validPurchaseEvents.includes(event)) {
      ...
      try {
        ...
        console.log(`✅ Usuário ${result.username} processado para ${buyer.email}`);
    ...
    // Processa eventos de cancelamento/reembolso
      ...
      try {
        ...
        if (assinatura) {
          ...
          console.log(`ℹ️ Assinatura não encontrada para transação ${purchase.transaction}`);
        }
        ...
        // SEMPRE retornar 200 OK ao Hotmart, mesmo em caso de erro
        return res.status(200).json({
          message: 'Webhook received but cancellation processing failed',
          error: error instanceof Error ? error.message : 'Unknown error',
          transaction_id: data?.purchase?.transaction || 'unknown'
        });
      }
